...
print("\n")
print(colored("OB DOWNLOADER UTILITIES",'red')+"\n")

#History Import
def his_imp():
    des_path = '/data/data/com.termux/files/home/history.txt'
    imp_path = '/storage/emulated/0/OB_Downloader/'
    def x(imp_path):
        ...
        return False
    if not x(imp_path):
         print("No History File to import available in OB Downloader folder!!")
         exit()
    
    print("\nKeep only one history Text file in OB Downloader folder and click enter !!\n")
    if input("") == "":
...
# Backup History
    ...
    dest = '/storage/emulated/0/OB_Downloader/history_backup_'+datetime.now().strftime("%d%m%y_%H%M%S")+".txt'"
...
#Developer info
    ...
    if i == "1":
        ...
        os.system("termux-open-url https://github.com/DhineshCool/OB-Downloader-Android")
    elif i == "4":
        os.system("termux-open-url https://github.com/DhineshCool/OB-Downloader-Android/issues")
    else:
         exit()
